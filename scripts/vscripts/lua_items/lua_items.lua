...
LinkLuaModifier("modifier_demonic_boots", "lua_items/lua_items.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_demonic_boots_passive", "lua_items/lua_items.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_demonic_boots:GetModifierMoveSpeed_Limit()
	if self:GetParent():HasModifier("modifier_sonic_super_speed") then
		return nil
	end
	return 660
end
...
function modifier_demonic_boots_passive:GetModifierPreAttack_BonusDamage()
	if self:GetParent():IsRangedAttacker() then
		return self.bonus_damage_range
	end
	return self.bonus_damage_melee
end