...
LinkLuaModifier( "modifier_meat_hook_followthrough_lua", "ability/modifier_meat_hook_followthrough_lua.lua" ,LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_meat_hook_lua", "ability/modifier_meat_hook_lua.lua" ,LUA_MODIFIER_MOTION_HORIZONTAL )
...
function pudge_meat_hook_lua:OnSpellStart()
	...
	EmitSoundOn( "SoundHookOn", self:GetCaster() )
...
function pudge_meat_hook_lua:OnProjectileHit( hTarget, vLocation )
	...
	if self.bRetracting == false then
		...
		if hTarget ~= nil then
			EmitSoundOn( "SoundGetOver", hTarget )
